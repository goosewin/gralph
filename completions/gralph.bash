_gralph() {
    ...
    do
        case "${cmd},${i}" in
            ...
            gralph__help,stop)
                ...
                ;;
            gralph__help,update)
                cmd="gralph__help__update"
                ;;
            gralph__help,verifier)
                cmd="gralph__help__verifier"
                ;;
    ...
    case "${cmd}" in
        ...
        gralph__prd__check)
            ...
            ;;
        gralph__prd__create)
            opts="-o -b -m -h --dir --output --goal --constraints --context --sources --backend --model --variant --max-retries --allow-missing-context --multiline --no-interactive --interactive --force --help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 3 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        gralph__update)
            ...
            ;;
        gralph__verifier)
            opts="-h --test-command --coverage-command --coverage-min --help [DIR]"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
