...
func New() *Backend {
	return &Backend{execPath: "opencode"}
}

var _ backend.Backend = (*Backend)(nil)

func init() {
	if err := backend.Register("opencode", New()); err != nil {
		panic(err)
	}
}
...
func (b *Backend) CheckInstalled() error {
	if strings.TrimSpace(b.execPath) == "" {
		return errors.New("opencode executable path is empty")
	}
	if _, err := exec.LookPath(b.execPath); err != nil {
		return fmt.Errorf("opencode not installed: %w", err)
	}
	return nil
}
...
func (b *Backend) RunIteration(ctx context.Context, opts backend.IterationOptions) error {
	...
	if err := cmd.Run(); err != nil {
		if stderr.Len() > 0 {
			return fmt.Errorf("opencode failed: %w: %s", err, strings.TrimSpace(stderr.String()))
		}
